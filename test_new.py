...
output_folder = f"../data/final/{past_len}_{future_len}_new"
...
for f in os.listdir(test_folder):
    ...
    while True:
        ...
        if len(buffer) == 0:        # first frame - no velocity no acceleration
            velocity = np.zeros(2)
            acceleration = np.zeros(2)
        elif len(buffer) == 1:      # second frame - velocity but no acceleration
            prev_center = buffer[-1][:2]
            velocity = center_norm - prev_center
            acceleration = np.zeros(2)
        else:                       # all other frames
            prev_center = buffer[-1][:2]
            prev_velocity = buffer[-1][2:4]
